...
- **Storage credential minting** — the source handler is the only service
  authorized to call the storage STS. It mints two credential sets per run,
  both scoped to `pipeline/{tenant}/{run-id}/*`: short-lived read/write for
  pipeline stages, and long-lived delete-only for cleanup. Downstream stages use
